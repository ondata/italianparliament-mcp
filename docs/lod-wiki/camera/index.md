...
* [Ordini del giorno presenti come nodi ma senza contenuto](aic-odg-senza-contenuto.md) - 4.944 ODG hanno solo tipo e collegamento all'atto, senza label/data/testo/firmatario: il tool `aic` li esclude (giustamente), quindi su **520 atti** la conta degli ordini del giorno è più bassa del reale, con punte di 125 su un solo atto. Sono reali — su `ac18_1117` i 125 muti hanno 125 votazioni che li citano, col proponente nella descrizione. Per contarli davvero passare dalle votazioni.
...
* [Il dataset AIC contiene anche il sindacato ispettivo del Senato](aic-ramo-senato.md) - 160.746 atti con URI a suffisso `_S` e `ocd:ramo` = "Senato della Repubblica", con testo integrale in `dc:description`, soggetti EuroVoc e primo firmatario su `senatore.rdf`. È la via per cercare per ARGOMENTO le interrogazioni dei senatori, che il LOD Senato non permette. Trappola sull'URL umano della scheda.
* [Iter di un atto — timeline degli stati](iter-statoIter.md) - la cronologia dell'iter è una timeline via `ocd:rif_statoIter` (uno stato per fase, `dc:date`+`dc:title`); copertura alla pari tra legislature (18 = 19). Il ramo Senato dà per design solo lo stato corrente (timeline nel feed RSS), asimmetria di ramo non di legislatura. **Trappola navetta**: la lettura successiva di un testo tornato modificato sta su un atto **variante** (`ac19_703` → `ac19_703-B`), non collegato da alcun triple, quindi la timeline dell'atto base sembra conclusa mentre la legge è stata approvata anni dopo; `-A` (4.558 atti) è il testo della commissione, non una lettura di navetta.
...
* [Nome d'uso e nome anagrafico](nome-uso-nickname.md) - il cognome noto (Casellati, Villecco Calipari) non è in `foaf:surname` né nella `rdfs:label` della `ocd:persona` ma dentro un blank node `foaf:nickname`; le `ocd:deputato` usano invece già il nome d'uso nella label. Cercare "casellati" sulla persona dà zero righe. 135 persone, 12 membri di governo; lato Senato il problema non esiste.
