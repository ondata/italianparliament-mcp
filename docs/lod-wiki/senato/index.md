...
* [Primo firmatario del sindacato ispettivo — sta nel suffisso dell'iniziativa](sindacato-ispettivo-primo-firmatario.md) - sugli atti di sindacato ispettivo `osr:primoFirmatario` **non esiste** (zero atti in leg. 19): l'ordine di firma è nel suffisso numerico del nodo iniziativa (`…-1`, `-2`, `-3`), e il `-1` c'è su tutti i 6.422 atti che hanno almeno un firmatario. Aggregare con `MIN(?presentatore)` e `MIN(?senatore)` accoppia il nome di un firmatario all'URI di un altro. Niente `BIND` né subquery: Virtuoso li rifiuta.
...
* [Presenze e assenze — l'assenza non è nel dato](presenze-assenze.md) - le cinque proprietà del voto (favorevole/contrario/astenuto/presenteNonVotante/inCongedoMissione) non comprendono l'**assente semplice**: l'assenza si ricava per differenza da un denominatore, che va limitato al **periodo di mandato** (`osr:mandato` → `osr:inizio`/`osr:fine`), altrimenti chi subentra risulta assente per i mesi in cui non era in carica (Gaudiano: 1.731 votazioni invece di 8.102, presenza 96% invece di ~20%). Le **missioni non sono assenze**: Segre ha il 99,27% di congedi e lo 0,01% di assenze. Il contatore `osr:presenti` **non** è la presenza in senso Openpolis (misurato su leg. 19: eccede i votanti di 10.512 mentre i presenti non votanti sono 61.529). Percentuali confrontabili con Openpolis entro circa un punto, non identiche.
