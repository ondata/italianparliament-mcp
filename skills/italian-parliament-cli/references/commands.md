...
> Un `--uri`/`--ddl-uri` che non sia di `dati.camera.it` o `dati.senato.it` è un **errore**: il tool instrada per host, e un URI di altro dominio prima veniva scartato in silenzio restituendo l'elenco intero del repertorio (con `--count-only`, l'intero corpus) come se fosse la risposta.
...
> **Navetta lato Camera: la lettura finale vive su un atto variante, non sull'originale.** Quando un testo torna dal Senato modificato, la nuova lettura riceve un atto **separato** con suffisso (`C.703` → `C.703-B`, poi `-C`, `-D`…). La timeline di `C.703` si ferma a `Approvato, segue Navette` il **28/02/2024** anche se la legge quadro sugli interporti è stata approvata definitivamente il **13/11/2025** (Legge 177/2025) su `C.703-B`: leggere quell'ultimo stato come stato attuale è l'errore tipico. Non serve più indovinarlo — sul ramo Camera il comando sonda da sé le letture successive e, se esistono, stampa un `NOTA:` su stderr con l'URI da rilanciare. Regola: **finché quella nota compare, l'iter che hai in mano non è finito.** L'atto variante si interroga come qualsiasi altro (`--uri http://dati.camera.it/ocd/attocamera.rdf/ac19_703-B`), e vale anche per gli altri comandi: `bill-rapporteurs`, `bill-signatories`, `bill-text links` e `camera-amendments` accettano l'URI con suffisso. Attenzione a non confondere i suffissi: `-B`/`-C`/`-D` sono le **letture successive**, `-A` è il **testo della commissione** (alla fonte sono 4.558 atti contro 1.807 `-B`) e non è affatto un'approvazione definitiva.
...
```
Colonne `bill_number` (numero DDL dal label, es. `562-B`; se il label è generico o col refuso, da v0.23.0 è backfillato dalla `osr:fase` del DDL risolto), `ddl_uri` (URI DDL/documento, popolato anche per le fiducie prive di `osr:oggetto` risolvendo il numero via `osr:fase`; da v0.26.0 `--ddl-uri` include le fiducie anche quando cadono su una seduta diversa dal voto fortemente collegato) e `ddl_title` (da v0.26.0: titolo del DDL/documento collegato, `osr:titoloBreve` con fallback `osr:titolo` — utile per voti con label generico, es. un ODG su una risoluzione di commissione; vuoto se `ddl_uri` è multiplo). Il tipo semantico vive nel `rdfs:label` (non in `osr:tipoVotazione`): `--confidence-vote true|false` (fiducia, esclude le mozioni di sfiducia) e `--final-vote true|false` (`Votazione finale`) sono label-based. `--keyword` cerca nel label del voto **e** (da v0.20.0) nel titolo del DDL/documento collegato (`osr:oggetto`/`osr:relativoA`/`osr:titolo` e, da v0.26.0, `osr:titoloBreve`), in OR; da v0.23.0 copre anche le **fiducie** (prive di `osr:oggetto`): il DDL citato per numero nel label viene risolto e il suo titolo concorre al match (es. `--keyword sicurezza` trova la fiducia sul decreto sicurezza). Limite residuo: fiducie con refuso nel numero del label e voti senza alcun DDL — usare `--ddl-uri` + `bill-progress`.
