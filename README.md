...
| `votes list` | Votazioni Camera con contatori (favorevoli, contrari, astenuti), filtrabile per data, tipo fiducia (`--confidence-vote`), atto collegato (`--bill-code`, il numero dell'**atto Camera** — il DL 100/2026 è il C.3053 — varianti incluse: `2790` prende anche il 2790-bis). Per mozioni e risoluzioni espone il codice AIC (`aic_code`) e il link alla scheda (`aic_link`) |
| `vote-detail show` | Come ha votato ogni singolo deputato in una votazione, con nome e gruppo |
| `attendance show` | Presenze e assenze di un deputato nelle votazioni d'Assemblea della sua legislatura: conteggi per esito e, dentro il "non ha votato", le tre situazioni che il dato Camera tiene distinte — `in_missione`, `presidente_di_turno` e `assenze` (l'assenza vera) — più le percentuali di presenza, missione e assenza. **`non_ha_votato` non è il numero di assenze**: per un ministro o un vicepresidente d'Assemblea è quasi tutto missioni e turni di presidenza (Mulè: 18.844 voti non espressi, di cui 546 assenze reali). Il denominatore è già delimitato al mandato, quindi chi subentra a legislatura iniziata non risulta assente per il periodo in cui non sedeva |
...
| `senato-votes list` | Votazioni d'Assemblea del Senato con esito, contatori (favorevoli/contrari/astenuti), tipo, data, DDL collegato e relativo titolo (`ddl_title`). Filtrabile per data, DDL, parola chiave nel label o nel titolo del DDL/documento collegato (`--keyword`), voti di fiducia (`--confidence-vote`) e voti finali (`--final-vote`). `--legislature` è facoltativa: viene dedotta dalle sedute che cadono nell'intervallo di date, o dal DDL |
...
| `senato-attendance show` | Presenze e assenze di un senatore nelle votazioni d'Assemblea di una legislatura: conteggi per categoria (favorevole/contrario/astenuto/presente non votante/in congedo o missione) e, con la formula di Openpolis, le percentuali di presenza, missione e assenza. Il denominatore è il numero di votazioni cadute dentro il mandato di quel senatore, così chi subentra o cessa a metà legislatura non risulta assente per il periodo in cui non era in carica. Le percentuali sono confrontabili con quelle pubblicate da Openpolis e dai giornali entro circa un punto. `--legislature` è facoltativa: se omessa viene dedotta dalle legislature in cui quel senatore ha votato, quindi funziona anche per chi non è più in carica |
...
| `audizioni list` | **[CAMERA]** Audizioni delle commissioni: data, commissione, titolo (con nome/ruolo dell'audito), atti collegati e link al bollettino. Filtrabile per `--committee-name`, `--keyword`, data; `--legislature` è facoltativa e viene dedotta dall'intervallo di date. `--count-only` per il solo totale (in leg. 14 non si combina con le date). Leg. 19 (dato vivo) via titolo della discussione; leg. 14 (storica) via `dc:type`; le altre via titolo (best-effort, ma il dato c'è: 6.400 audizioni in leg. 18). Senato non coperto (via SPARQL non espone data né commissione delle audizioni) |
...
- **Gli interventi di un deputato comprendono quelli da membro del governo** (da **v0.36.1**): alla Camera chi parla da presidente del Consiglio, ministro o sottosegretario è registrato sull'incarico di governo, non sulla scheda di deputato, e `speeches list --deputy-uri` finora non lo vedeva. Giorgia Meloni nella legislatura 19 risultava con **1 intervento invece di 123**, Gilberto Pichetto Fratin con 1 invece di 135. Chi ha citato un conteggio di interventi di un parlamentare con incarichi di governo deve rifarlo. La colonna `gov_member_uri` indica quali righe sono da governo. Due limiti della fonte: i membri del governo **non parlamentari** (per esempio Matteo Piantedosi o Guido Crosetto) non hanno interventi nei dati della Camera, anche se in Aula hanno parlato, quindi uno zero non significa silenzio; e gli interventi pronunciati come **presidente di turno** non sono inclusi, perché non sono collegati al deputato e sono per lo più atti di conduzione della seduta.
- **Non serve indicare la legislatura quando un altro filtro la determina** (da **v0.31.0**): cercare per date del 2020 rispondeva "nessun risultato" perché sotto sotto veniva interrogata la legislatura in corso, e quel vuoto si leggeva come assenza del dato. Ora `senato-votes` la deduce dalle sedute dell'intervallo, `audizioni` dalle date, `senato-attendance` dalle legislature in cui quel senatore ha votato: le due risoluzioni sul MES del 9 dicembre 2020, le 6.400 audizioni della legislatura 18 e le presenze di chi non siede più in Senato sono raggiungibili senza sapere in anticipo di quale legislatura si tratti. Se l'intervallo di date copre due legislature il comando si ferma e chiede quale, invece di rispondere con una sola: lo stesso numero d'atto esiste in entrambe e i risultati non sarebbero confrontabili. Fa eccezione `group-rank`, dove non c'è alcun filtro da cui dedurre e la legislatura passata va indicata.
- **Un'opzione scritta male è un errore, non un filtro ignorato** (da **v0.31.0**): prima `--committee-uri` al posto di `--committee-name` restituiva un elenco normale, senza il filtro che si credeva di aver applicato. Ora la CLI si ferma, dice quale opzione non conosce e suggerisce quella giusta.
- **Un flag booleano senza valore è un errore, non un filtro assente** (da **v0.33.0**): `votes list --legislature 19 --confidence-vote` — la cosa più naturale da scrivere — restituiva **19.428 votazioni invece di 71**, perché il flag nudo veniva letto come "opzione non passata" e il filtro non veniva mai applicato. Ora la CLI si ferma e dice cosa scrivere (`--confidence-vote true`). Vale per tutti i flag booleani.
- **I filtri per sottostringa battono sul valore, non sull'indirizzo del dato** (da **v0.33.0**): `bills --natura` e `deputies --birth-place` filtrano proprietà che nel grafo sono URI (`.../natura.rdf/disegno_legge_ordinario`, `.../luogo.rdf/genova_genova_liguria`), e il confronto cadeva sull'URI intero. Così `--natura camera` restituiva tutti i 3.107 atti della legislatura 19 e `--birth-place camera` tutti i 414 deputati con un luogo di nascita: non zero righe — che l'avviso di risultato vuoto avrebbe saputo spiegare — ma l'intero elenco, indistinguibile da "nessun filtro". Ora il confronto è ancorato al codice leggibile, quello che vedi in colonna.
- **`bills` restituisce una riga per atto** (da **v0.33.0**): gli atti con più di un primo firmatario — 15.514, tutti in legislature storiche — producevano una riga per firmatario. Su 1.000 righe c'erano 557 atti distinti: il budget di `--limit` si consumava sui duplicati, `--offset` saltava atti e `--count-only` senza filtro di legislatura gonfiava del 32% (160.454 contro 121.021). I conteggi filtrati per legislatura erano invece sempre corretti, ed è per questo che il difetto è rimasto a lungo invisibile.
- **Le interrogazioni dei senatori si cercano per argomento con `aic`, non con `sindacato-ispettivo`** (da **v0.30.0**): il dataset degli atti di indirizzo e controllo della Camera pubblica **anche** il sindacato ispettivo del Senato — 160.746 atti, con il **testo integrale** — mentre il repertorio del Senato ne espone solo tipo, numero, data e firmatari. Quindi «cosa hanno chiesto i senatori sul nucleare?» ha risposta: `aic list --chamber senato --keyword nucleare`. La colonna `chamber` dice il ramo di ogni riga. Due avvertenze: in legislatura 17 il 4,5% degli atti non dichiara il ramo e il filtro `--chamber` li esclude; per gli atti del Senato `html_url` è vuoto (nessun pattern di scheda verificabile) e il riferimento navigabile è la colonna `url`, che porta al PDF ufficiale.
- **Il numero di un atto cambia passando da una camera all'altra** (da **v0.30.0**): la delega sul nucleare è `C.2669` alla Camera e `S.1924` al Senato. Cercare il numero della Camera sul Senato non trova nulla — e prima quel vuoto sembrava dire "il Senato non ha ancora l'atto". Ora `bill-progress list --number 2669 --branch S` risale da sé al provvedimento e restituisce **tutte le sue fasi nelle due camere**, in ordine di iter, spiegando perché il numero mostrato è un altro. Vale anche per le navette a più letture (il DDL sul voto ai diciottenni torna come `C.1511 → S.1440 → C.1511-…-B → S.1440-B`). Attenzione al caso opposto: quando un DDL con quel numero al Senato esiste davvero, è spesso un atto **completamente diverso** — controlla che il titolo coincida.
- **Un elenco vuoto su un intervallo di date dice se il dato è in ritardo o assente** (da **v0.30.0**): i dati della Camera vengono pubblicati a lotti, e ogni record porta la data del proprio caricamento. Quando `aic` o `votes` non restituiscono nulla su un filtro per date, l'avviso dice fino a quando quell'area risulta caricata e distingue i due casi: la finestra chiesta **non è ancora coperta** (può essere dato non ancora pubblicato: verifica alla fonte, non concludere che non sia avvenuto nulla) oppure **è coperta** (il ritardo è escluso, la causa sta nei filtri o è un'assenza strutturale). Le aree hanno freschezze molto diverse: a fine luglio 2026 atti e interrogazioni erano aggiornati al 28 luglio, le votazioni al 24, mentre sedute e interventi d'Aula erano fermi al 18 giugno. Il Senato non espone questo segnale.
- **Elenchi troncati da `--limit`** (da **v0.29.0**): quando un elenco riempie il limite, altre righe che soddisfano i filtri possono essere rimaste fuori — e l'ordinamento è per data decrescente, quindi su un filtro per intervallo a mancare sono i **giorni più vecchi**. Prima l'elenco tagliato era indistinguibile da uno completo: `votes list --date-from 2020-02-24 --date-to 2020-02-28` restituiva 100 righe tutte del 27/2, facendo sembrare che il 25/2 (fiducia sul decreto intercettazioni) non si fosse votato. Ora compare un avviso — su `stderr` per la CLI, così `| jq` e `> file.csv` restano intatti; in coda alla risposta per l'MCP. Usa `--count-only` (dove disponibile) per il totale reale prima di concludere: su intervalli lunghi il massimo di `--limit` può coprire un solo mese (le AIC del 2025 sono 11.649).
- **Voti per alzata di mano al Senato** (`senato-votes`): l'Assemblea del Senato vota *normalmente* per alzata di mano (art. 113 c.2 del Regolamento), e quel voto non produce conteggi: non esiste come votazione nei dati, che espongono le sole votazioni con rilevazione dei voti (elettroniche, nominali con appello, segrete), come dichiarano le pagine "Votazione" di senato.it. Un provvedimento può quindi risultare approvato senza che `senato-votes` mostri nulla: non è un dato mancante, è un voto senza numeri. L'esito resta leggibile da `bill-progress` (`status` e `status_date`). Fanno eccezione i voti finali su leggi costituzionali, elettorali, di delega, di bilancio e di conversione in materia di ordine pubblico, sempre elettronici per regolamento (art. 120 c.3).
