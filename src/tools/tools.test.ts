...
// ---------------------------------------------------------------------------
// Trasferiti da `committee-sessions.intervento-null.test.ts`: erano test LIVE
// (ogni `execute()` interroga dati.senato.it) in un file che il job CI
// "Deterministic tests (no live SPARQL)" NON escludeva. Risultato: la CI
// diventava rossa a caso, quando il runner GitHub incassava un 403
// dall'endpoint, su PR che non c'entravano nulla — un segnale di fallimento
// che indicava il colpevole sbagliato. I test live stanno qui, che è l'unico
// file (con sparql.test.ts) che la CI esclude.
// ---------------------------------------------------------------------------
